#include "gbwt_extender_no_cache.hpp"
...
void match_initial(GaplessExtension& match, const std::string& seq, gbwtgraph::view_type target) {
    ...
    while (left > 0) {
        ...
        if (a == b) {
            ...
            for (size_t i = 0; i < len; i++) {
                // std::cerr << "[INITIAL] Seq: " << seq[match.read_interval.second] << " Target: " << target.first[node_offset] << std::endl;
    ...
    // std::cerr << "Read interval " << match.read_interval.first << " " << match.read_interval.second << std::endl;
...
size_t match_forward(GaplessExtension& match, const std::string& seq, gbwtgraph::view_type target, uint32_t mismatch_limit) {
            ...
            for (size_t i = 0; i < len; i++) {
                if (seq[match.read_interval.second] != target.first[node_offset]) {
                    if (match.internal_score + 1 >= mismatch_limit) {
                        // std::cerr << "If: " << node_offset << std::endl;
                        return node_offset;
                    }
                    match.internal_score++;
                }
    ...
    // std::cerr << "Full return " << node_offset << std::endl;
    return node_offset;
}
...
void handle_full_length(const HandleGraph& graph, std::vector<GaplessExtension>& result, double overlap_threshold) {
    ...
    //     std::cerr << "Result " << i << " path " << result[i].path.size() << std::endl;
    // }
...
// Test
bool trim_mismatches(GaplessExtension& extension, const gbwtgraph::GBWTGraph& graph, const Aligner& aligner) {
...
std::vector<GaplessExtension> GaplessExtender::extend(cluster_type& cluster, std::string sequence, const gbwtgraph::CachedGBWTGraph* cache, size_t max_mismatches, double overlap_threshold) const {
    ...
    // cout << "Entrou extend no cache" << endl;
    ...
    // std::cerr << "Data: " << sequence << std::endl;
    for (seed_type seed : cluster) {
        // std::cerr << "Seed: " << seed.first.data << " " << seed.second << std::endl;
        ...
        if (best_alignment < result.size() && result[best_alignment].internal_score == 0) {
            if (result[best_alignment].contains(*this->graph, seed)) {
                // std::cerr << "Contains on result" << std::endl;
                continue;
            }
        }
        ...
        while (!extensions.empty()) {
            ...
            if (best_match < curr) {
                ...
                // std::cerr << "Best match " << best_match.path.size() << std::endl;
            }
        }
        ...
        if (!best_match.empty()) {
            ...
            // std::cerr << "Update result - best match path " << best_match.path.size() << std::endl;
    ...
    if (best_alignment < result.size() && result[best_alignment].internal_score <= max_mismatches) {
        handle_full_length(*this->graph, result, overlap_threshold);
